  ...
  const handleGenerateClick = () => {
    ...
    fetch(`${ENDPOINT}/scenario/generate_scenario`, {
      ...
      .then((data) => {
        ...
        setInteractiveQuestions(data.interactive_questions || []); // Set to empty array if not defined
  ...
  const formatQuestions = (questions) => {
    return questions.map((question, index) => {
      ...
      if (question.includes('$-')) {
        return (
          <li key={index} style={{ color: '#00ffff', fontWeight: 'bold' }}>
            {question.replace('$-', '')} {/* Remove the $- */}
          </li>
        );
      } else {
    ...
    <div className="scenario-container">
      ...
      <div className="scenario-output-container">
        ...
        <div className="scenario-output-box">
          {generatedScenario ? generatedScenario : "Your generated scenario will appear here :)"}
        </div>
